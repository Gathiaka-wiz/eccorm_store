	...
	return (
		...
		>
			<ul className="flex px-2 py-2.5 justify-between w-screen sm:justify-evenly">
				...
				<form
					className="w-5/12 h-8 relative flex align-middle justify-center  sm:h-10 "
					onSubmit={handleSearch}
				>
					<SearchIcon
						className="ml-1 absolute left-0 top-0 bottom-0 text-white w-5 h-8 z-10 sm:h-10 sm:w-6 md:ml-5  "
						type="submit"
				...
				{isAuthenticated ? (
					<>
						<li className=" flex justify-center align-center relative w-10 h-8 cursor-pointer ">
							<ShoppingCart
								className="w-6 h-9 sm:w-9 cursor-pointer "
								onClick={handleCart}
							/>
							<span
								className="absolute -top-1 right-0 bg-red-500 rounded-xl px-1.5 inline-block text-white text-sm font-[supreme-bold] cursor-pointer "
								onClick={handleCart}
							>
						...
						<li className="flex justify-center align-center cursor-pointer ">
							<User
								className="w-8 h-7 sm:w-15 sm:h-9 cursor-pointer "
					...
					<div>
						<button
							className=" bg-[#ff6214] h-8 text-white font-[Supreme-Regular] text-sm px-1 rounded cursor-pointer sm:w-30 sm:h-9 md:text-lg md:rounded-[5.5px] md:w-32 "
