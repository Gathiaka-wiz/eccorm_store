...
export const userStore = create((set) => ({
	...
	getUserOrders: async () => {
		set({ isFetching: true, error: null, message: null, orders: null });

		try {
			const data = await apiRequest(`get`, `${API_URL}/user/orders`);
			...
			set({
				error: error?.data?.message || 'User orders fetch error',
				isFetching: false,
			});
			throw error;
		}
	...
	addAndUpdateCart: async (product_id, quantity) => {
		...
		try {
			const data = await apiRequest(
				'post',
				`${API_URL}/product/${product_id}/add-to-cart`,
				{ quantity }
			);
	...
	updateCartItem: async (product_id, quantity) => {
		...
		try {
			const data = await apiRequest(
				'patch',
				`${API_URL}/user/cart/${product_id}/update-product`,
				{ quantity }
			);
	...
	deleteCartItem: async (product_id) => {
		...
		try {
			const data = await apiRequest(
				'delete',
				`${API_URL}/user/${product_id}/delete-from-cart`
			);
